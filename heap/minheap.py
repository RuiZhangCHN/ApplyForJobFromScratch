...
class MinHeap(object):
    ...
    def _shiftdown(self, index):
        ...
        while child_idx < self.size():
            if child_idx+1 < self.size() and self._data[child_idx+1] < self._data[child_idx]:
                child_idx += 1

            if self._data[index] < self._data[child_idx]:
                break

            self._data[index], self._data[child_idx] = self._data[child_idx], self._data[index]
            index = child_idx
            child_idx = (index << 1) + 1
...
def testIntValue():
    for iTimes in range(10):
        ...
        oMaxHeap = MinHeap()
        print('_data:\t   ', allData)
        arrDataSorted = sorted(allData, reverse=False)
...
def testTupleValue():
    for iTimes in range(10):
        ...
        oMaxHeap = MinHeap()
        arrDataSorted = sorted(allData.items(), key=lambda d: d[0], reverse=False)
